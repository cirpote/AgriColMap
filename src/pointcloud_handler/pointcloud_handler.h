...
class PointCloudHandler{
    ...
    public:
        ...
        void transformPointCloud( const Matrix4& tf,
                                  const std::string& cloud_to_scale,
                                  const std::string& cloud_type );
    ...
    protected:
        ...
        // Algorithm Variables
        std::shared_ptr<open3d::PointCloud> horiz_cloud;
        PCLXYZRGB_Map pclMap, pclMapFiltered, pclMapFilteredDownSampled;
        std::unordered_map< std::string, Vector3d> initGuessTMap;
        std::unordered_map< std::string, Vector3d> initGuessQMap;
